...
import { LayeredRenderer } from './rendering/layered-renderer.js';
...
class GameApp {
  ...
  fitCanvas() {
    ...
    if (this.layeredRenderer) {
      this.layeredRenderer.handleResize(rect.width, rect.height);
    }
  ...
  initPerformanceModules() {
    ...
    this.camera = new Camera({
      x: 0,
      y: 0,
      viewportWidth: this.VIEW.w,
      viewportHeight: this.VIEW.h,
      worldWidth: 2000,
      worldHeight: 2000
    });
    
    // 初始化分层渲染器
    this.layeredRenderer = new LayeredRenderer(this.canvas, this.ctx, {
    ...
    console.log('[GameApp] 性能优化模块初始化完成');
  }
  ...
  registerStateReducers() {
    ...
    // 世界状态管理
    this.stateManager.registerReducer('world:updateTimer', (state, action) => {
      const { deltaTime } = action.payload;
      return {
        ...state,
        world: {
          ...state.world,
          timer: state.world.timer + deltaTime
        }
      };
    });
  ...
  updateGame(dt) {
    ...
    // 更新摄像机 - 使用新的Camera类
    if (this.camera) {
      this.camera.setFollowTarget(window.G.player);
      this.camera.update(dt);
      
      // 同步到旧的camera对象以保持兼容性
      const cameraPos = this.camera.getPosition();
      window.G.camera.x = cameraPos.x;
      window.G.camera.y = cameraPos.y;
    } else {
      // 回退到旧的摄像机逻辑
    ...
    // 更新分层渲染器
    if (this.layeredRenderer) {
      this.layeredRenderer.update(dt);
    }
  }
  ...
  renderGame() {
    if (this.layeredRenderer) {
      // 使用分层渲染器
      this.layeredRenderer.render(this.camera);
      
      // 绘制临时游戏对象
      this.renderTemporaryObjects();
    } else {
      // 回退到传统渲染
  ...
  drawUI() {
    ...
    // 基础性能信息
    this.ctx.fillText(`FPS: ${metrics.current.fps.toFixed(1)} (${metrics.fps.min.toFixed(1)}-${metrics.fps.max.toFixed(1)})`, 15, y); y += 15;
    this.ctx.fillText(`Frame: ${metrics.current.frameTime.toFixed(1)}ms`, 15, y); y += 15;
    this.ctx.fillText(`Memory: ${(metrics.memory.current / 1024 / 1024).toFixed(1)}MB`, 15, y); y += 15;
    
    // 游戏状态信息
    this.ctx.fillText(`Player: ${Math.round(window.G.player.x)}, ${Math.round(window.G.player.y)}`, 15, y); y += 15;
    this.ctx.fillText(`Camera: ${Math.round(window.G.camera.x)}, ${Math.round(window.G.camera.y)}`, 15, y); y += 15;
    this.ctx.fillText(`Status: ${state.gameStatus}`, 15, y); y += 15;
    
    // 渲染统计
    if (this.layeredRenderer) {
      const renderStats = this.layeredRenderer.getStats();
      this.ctx.fillText(`Layers: ${renderStats.layersRendered}/${renderStats.totalLayers}`, 15, y); y += 15;
      this.ctx.fillText(`Objects: ${renderStats.objectsRendered}/${renderStats.totalObjects}`, 15, y); y += 15;
      this.ctx.fillText(`Dirty Rects: ${renderStats.dirtyRects}`, 15, y);
    }
  }
  ...
  async runPerformanceBenchmark() {
    if (!this.performanceBenchmark) {
      console.warn('[GameApp] 性能基准测试未初始化');
      return null;
    }
    
    console.log('[GameApp] 开始性能基准测试...');
    
    try {
      const results = await this.performanceBenchmark.runTestSuite();
      
      console.log('[GameApp] 性能基准测试完成:');
      console.log('总体得分:', results.overallScore);
      console.log('详细结果:', results);
      
      // 发布测试结果事件
      this.eventBus.emit('performance:benchmark:complete', results);
      
      return results;
    } catch (error) {
      console.error('[GameApp] 性能基准测试失败:', error);
      return null;
    }
  }
...
// 导出性能优化模块到全局
Object.defineProperty(window, 'layeredRenderer', {
  get: () => gameApp.layeredRenderer
});
Object.defineProperty(window, 'memoryManager', {
  get: () => gameApp.memoryManager
});
Object.defineProperty(window, 'performanceBenchmark', {
  get: () => gameApp.performanceBenchmark
});
...
// 添加全局快捷方法
window.runBenchmark = () => gameApp.runPerformanceBenchmark();
window.forceGC = () => gameApp.memoryManager?.forceGC();
window.getPerformanceReport = () => gameApp.performanceMonitor.getReport();
window.getMemoryReport = () => gameApp.memoryManager?.getReport();
window.getRenderStats = () => gameApp.layeredRenderer?.getStats();

console.log('[GameApp] 性能优化快捷键:');
console.log('  B - 运行性能基准测试');
console.log('  M - 手动触发垃圾回收');
console.log('  P - 暂停/恢复游戏');
console.log('[GameApp] 全局调试方法:');
console.log('  runBenchmark() - 运行性能基准测试');
console.log('  forceGC() - 强制垃圾回收');
console.log('  getPerformanceReport() - 获取性能报告');
console.log('  getMemoryReport() - 获取内存报告');
console.log('  getRenderStats() - 获取渲染统计');
